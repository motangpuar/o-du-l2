...
void readCuCfg()
{
   ...
   /* YMA, Allow CU Stub read xml file */
   ...
   sctp = &duCfgParam.sctpParams;
    ...
    uint32_t duIpInt = sctp->duIpAddr.ipV4Addr;
    uint32_t cuIpInt = sctp->cuIpAddr.ipV4Addr;

    struct in_addr du_ip_addr;
    du_ip_addr.s_addr = duIpInt;

    struct in_addr cu_ip_addr;
    cu_ip_addr.s_addr = cuIpInt;

   DU_LOG("DU IP Address %s\n", inet_ntoa(du_ip_addr));
   DU_LOG("CU IP Address %s\n", inet_ntoa(cu_ip_addr));
   // ============================================
   ...
   memset(&ipv4_cu, 0, sizeof(uint32_t));
   //cmInetAddr((S8*)LOCAL_IP_CU, &ipv4_cu);
   cmInetAddr((S8*)inet_ntoa(cu_ip_addr), &ipv4_cu);
   ...
   while(numDu < NUM_F1_ASSOC)
   {
      ...
      cmInetAddr((S8*)inet_ntoa(du_ip_addr), &ipv4_cu);
